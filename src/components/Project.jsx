...
export const Project = ({ images, projectName, desc, techStacks, projectLink }) => {
    ...
    return (
        ...
        >
            <div className='bg-gray-100 dark:bg-white/10 dark:hover:bg-white/20 max-w-[42rem] m-auto border border-black/5 rounded-lg overflow-hidden sm:pr-8 relative sm:h-[20rem] hover:bg-gray-200 transition'>
                <a href='/' className='project-github-link absolute flex justify-center items-center  right-1 w-[40px] h-[30px] rounded-md bg-[#1e2124] focus:scale-105 hover:scale-105 active:scale-100 transition text-lg group-odd:right-[initial] group-odd:left-1 top-1' target='_blank'><FaGithubSquare /></a>
                <div className='pt-4 pb-7 px-5 sm:pl-10 sm:pr-2 sm:pt-10 sm:max-w-[50%] flex flex-col h-full sm:group-even:ml-[20rem]'>
                    ...
                    <ul className='flex flex-wrap mt-4 gap-2 sm:mt-auto'>
                        {techStacks.map(techStack => (
                            <li className='project-tech-stack bg-black/[0.7] px-3 py-1 text-[0.7rem] uppercase tracking-wider text-white rounded-full '>{techStack}</li>
                        ))}
                    </ul>
                </div>
